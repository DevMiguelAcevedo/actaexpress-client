...
export default function CreateNewActaPage() {
  ...
  const filteredPeople =
    registeredUsers?.filter((person) => {
      const search = searchTerm.toLowerCase();
      return (
        person.name?.toLowerCase()?.includes(search) || // name en lugar de nombre
        person.cargo?.toLowerCase()?.includes(search) ||
  ...
  const validateForm = () => {
    ...
    if (form.participantes.length === 0) {
      ...
      setShowSearchModal(true); // Abre automáticamente el modal
      return false;
    }
  ...
  const handleGuardar = async () => {
    ...
    try {
      const actaData = {
        ...
        asistentes: form.participantes.map((p) => p.id), // Envía los IDs de los participantes
        ...
        notas: form.notas || "",
        estado: "pendiente",
        // El responsable se agregará automáticamente desde el contexto
        lugar: form.lugar || "", // Agrega estos campos a tu estado si no existen
        orden_dia: form.orden_dia || "",
        desarrollo: form.desarrollo || "",
        acuerdos: form.acuerdos || "",
        firmas: [],
      };

      console.log("Datos a enviar:", actaData); // Para depuración

      await createNewActa(actaData);
      navigate("/dashboard", {
        state: { success: "Acta creada exitosamente" },
      });
    } catch (error) {
      ...
      setSubmitError(
        error.response?.data?.message ||
          ...
          "Error al guardar el acta. Por favor intente nuevamente."
      );
    }
  };
